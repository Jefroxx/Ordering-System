            ...
            if(Status == "Add")
            {
                using (SqlConnection conn = db.GetConnection())
                {
                    using (SqlCommand cmd = new SqlCommand("AddEmployee", conn))
                    {
                        ...
                        if (result == 1)
                        {
                            MessageBox.Show("Employee " + Status + "ed Successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            this.Close();
                        }
                        else
                        {
                            MessageBox.Show("An employee with the same first and last name already exists!", "Duplicate", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    }
            ...
            else if (Status == "Edit")
            {
                using (SqlConnection conn = db.GetConnection())
                {
                    using (SqlCommand cmd = new SqlCommand("AddEmployee", conn))
                    {
                        ...
                        // Pass parameters
                        //cmd.Parameters.AddWithValue("@EmployeeID", Convert.ToInt32(txtEmployeeID.Text)); // must have this field
                        cmd.Parameters.AddWithValue("@Last_Name", txtLname.Text.Trim());
                        cmd.Parameters.AddWithValue("@First_Name", txtFname.Text.Trim());
                        cmd.Parameters.AddWithValue("@Middle_Initial", txtMI.Text.Trim());
                        cmd.Parameters.AddWithValue("@Birthday", birthdayPicker.Value.Date);
